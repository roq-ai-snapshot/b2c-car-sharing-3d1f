...
import { useCarLocationFindFirst } from 'lib/roq';

import { CarLocationInterface } from 'interfaces/car-location';

function CarLocationViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const queryParams = useMemo<{ include: { car: boolean; location: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useCarLocationFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('car_location', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/car-locations/edit/${id}`} passHref legacyBehavior>
                    <Button
